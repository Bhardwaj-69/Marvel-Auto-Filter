...
async def set_stream_link(client, message):
    try:
        ...
        if link:
            ...
            await message.reply("Usage: /stream https://t.me/LarvaLinks or http://t.me/DM_HUB_069")
    except IndexError:
        await message.reply("Usage: /stream https://t.me/LarvaLinks or http://t.me/DM_HUB_069")
        return
...
async def m_grp(client, message):
    links = []
    link = await client.ask(message.chat.id ,"send me your pm search grp link or send /skiplink to skip , default is DM_HUB_69")
    if link.text == "/skiplink":
        links.append("https://t.me/DM_HUB_69")
    else:
        if checkIfLinkIsValid(link.text):
            links.append(link.text)
        else:
            await message.reply("Invalid link")
    link1 = await client.ask(message.chat.id ,"send me your movies grp link or send /skiplink to skip . default is DM_HUB_69")
